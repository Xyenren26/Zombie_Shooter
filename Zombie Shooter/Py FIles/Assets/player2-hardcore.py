...
high_scores_file = "scores/Player2_Hardcore_scores.txt"
...
# Load player image and resize it to the appropriate size
player2_image_right = pygame.image.load("normal/player2_right.png")
player2_image_left = pygame.image.load("normal/player2_left.png")
player2_image_up = pygame.image.load("normal/player2_up.png")
player2_image_down = pygame.image.load("normal/player2_down.png")
...
while running:
    ...
    if not game_over:  # Check if the game is not over for player movement
        ...
        if keys[pygame.K_LEFT]:
            ...
            for rock_pos in rock_positions:
                rock_rect = pygame.Rect(rock_pos[0], rock_pos[1], rock_size, rock_size)
                if player_rect.colliderect(rock_rect):
                    player_rect.x += PLAYER1_SPEED  # Move the player back if colliding
        ...
        if keys[pygame.K_UP]:
            ...
            for rock_pos in rock_positions:
                rock_rect = pygame.Rect(rock_pos[0], rock_pos[1], rock_size, rock_size)
                if player_rect.colliderect(rock_rect):
                    player_rect.y += PLAYER1_SPEED  # Move the player back if colliding
        ...
        if keys[pygame.K_a]:
            ...
            for rock_pos in rock_positions:
                rock_rect = pygame.Rect(rock_pos[0], rock_pos[1], rock_size, rock_size)
                if player2_rect.colliderect(rock_rect):
                    player2_rect.x += PLAYER2_SPEED  # Move the player back if colliding
        if keys[pygame.K_d]:
            player2_rect.x += PLAYER2_SPEED
        ...
        if keys[pygame.K_w]:
            ...
            for rock_pos in rock_positions:
                rock_rect = pygame.Rect(rock_pos[0], rock_pos[1], rock_size, rock_size)
                if player2_rect.colliderect(rock_rect):
                    player2_rect.y += PLAYER2_SPEED  # Move the player back if colliding
        if keys[pygame.K_s]:
            player2_rect.y += PLAYER2_SPEED
   ...
   # Generate a power-up randomly, avoiding spawning near obstacles
    if not revive and current_time - revive_time > 30000:
        while True:
            ...
            revive_candidate = revive_image.get_rect(center=(random.randint(50, SCREEN_WIDTH - 50),
                                                                random.randint(50, SCREEN_HEIGHT - 50)))
            # Check if the power-up collides with any obstacle (rock)
            collides_with_rock = False
            for rock_pos in rock_positions:
                rock_x, rock_y, rock_size = rock_pos
                rock_rect = pygame.Rect(rock_x, rock_y, rock_size, rock_size)
                if revive_candidate.colliderect(rock_rect):
                    collides_with_rock = True
                    break
    ...
    # Check for collision between player2 and rock
    for rock_pos in rock_positions:
        rock_rect = pygame.Rect(rock_pos[0], rock_pos[1], 50, 50)  # Assuming rock size is 50x50
        if player2_rect.colliderect(rock_rect) and player2_alive:
            slowed_by_rock = True
