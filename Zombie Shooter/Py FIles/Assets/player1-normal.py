...
high_scores_file = "scores/Normal_scores.txt"
...
for _ in range(25):
    ...
    ice_size1 = random.randint(50, 100)  # Randomize the size of the ices
    ice_size = random.randint(50, 50)  # Randomize the size of the ices

    # Ensure ices do not spawn too close to the player or other ices
    while True:
        ice_x = random.randint(0, SCREEN_WIDTH - ice_size1)
        ice_y = random.randint(0, SCREEN_HEIGHT - ice_size1)
    ...
    ice_positions.append((ice_x, ice_y, ice_size1))  # Store the ice position and size
...
# Function to check if a position is too close to ices
def is_position_far_from_ices(x, y):
    for ice_pos in ice_positions:
        ice_x, ice_y, _ = ice_pos
        distance = math.sqrt((x - ice_x) ** 2 + (y - ice_y) ** 2)
        if distance < 100:  # Adjust this threshold as needed
            return False  # Position is too close to a ice
    return True  # Position is far enough from ices
...
while running:
    ...
    if not game_over:  # Check if the game is not over for player movement
        ...
        if keys[pygame.K_LEFT]:
            ...
            for ice_pos in ice_positions:
                ice_x, ice_y, ice_size = ice_pos  # Unpack the ice position and size
                ice_rect = pygame.Rect(ice_x, ice_y, ice_size, ice_size)
                if player_rect.colliderect(ice_rect):
                    player_rect.x -= PLAYER_SPEED  # speed up the player back if colliding
            ...
            for ice_pos in ice_positions:
                ...
                ice_rect = pygame.Rect(ice_x, ice_y, ice_size, ice_size)
                if player_rect.colliderect(ice_rect):
                    player_rect.y -= PLAYER_SPEED  # speed up the player back if colliding
    ...
    if current_time - spawn_ammo_box_time > 20000:  # Check if 20 seconds have passed
        while True:
            ...
            if is_position_far_from_ices(ammo_x, ammo_y):
   ...
   # Generate a power-up randomly, avoiding spawning near ices
    ...
    for bullet in bullets[:]:  # Iterate over a copy of the bullets list
        for ice_pos in ice_positions:
            ice_rect = pygame.Rect(ice_pos[0], ice_pos[1], ice_size1, ice_size1)
